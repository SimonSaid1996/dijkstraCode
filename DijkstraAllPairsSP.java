...
public class DijkstraAllPairsSP {
     ...
     */
    public DijkstraAllPairsSP(EdgeWeightedGraph G) {//0-4.1-4 all null for some reason, which is not supposed to happen
        all  = new DijkstraSP[G.V()];
        for (int v = 0; v < G.V(); v++){
			all[v] = new DijkstraSP(G, v);
			//System.out.println("all[v] is "+all[v]);
		}
		
    }

	//need to modify this code here so that i won't print out every information, modify it so that it only print out from one vertex to the other
	public void toString1(){//to print out all the pairs and its length
		for(int i=0;i<all.length;i++){
			for(int j=0;j<all.length;j++){
				System.out.println("connected edges are "+all[i].distTo(j)+" i is "+i+" j is "+j);
			}
		}
    ...
    public Iterable<Edge> path(int s, int t) {
        validateVertex(s);
        validateVertex(t);
		System.out.println("t is "+t+" s is "+s);
		//System.out.println("return value is "+all[t].pathTo(s)+" all is "+all[t]);
        return all[t].pathTo(s);
    }
